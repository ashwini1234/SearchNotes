...
public class Database {
	...
	/**
	 * if the search Criteria entered by user matches the keywords in any of the keys,
	 * then return all the values containing those keywords in keys
	 * 
	 ...
	 * 
	 * use matchLevel() from SearchCriteria
	 * make a list of keywords after matching, sort them using compare
	 * anonymous block:compare() for sorting
	 * return all values for the keywords list
	 * 
	 * 
	 * @param keyList
	 */
	public void searchNote(final SearchCriteria sc){
	...
	public static void printMap(HashMap<SearchCriteria, Note> map){
        ...
        for(SearchCriteria s:keys){
            System.out.println(s.getSearchList()+":"+map.get(new SearchCriteria(s.getId(),s.getSearchList())).getNote());
        }
	 }
...
/*public void searchNote(List<String> keyList){
	Set<SearchCriteria> keys = dataMap.keySet();
	for(String userSearch:keyList){
		for(SearchCriteria searchCriteria:  keys){
			for(String searchList:searchCriteria.getSearchList()){
				if(userSearch.equalsIgnoreCase(searchList)){
					//return  
				}
		}
	}
